...
tags: ["query:re-us"]
score: 7.0
evidence: 将高级需求与低级测试链接以进行验证与确认
tldr: "针对现代软件开发中高层需求与底层测试脱节的问题，本文提出了开源框架 VNVSpec。该框架通过使验证与确认（V&V）规范机器可读且可执行，实现了从高层需求到模块化指标的分解，并利用可追溯图链接测试结果，自动生成审计报告。该方法在 AI 和信息物理系统中具有重要应用价值，能有效提供合规性证据并支持自动化审计。"
source: arxiv
