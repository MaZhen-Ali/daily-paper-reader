...
figures_json: "[{\"url\": \"assets/figures/arxiv/2603.20534v1/fig-001.webp\", \"caption\": \"Fig. 1. Production-grade RAG framework architecture with six integrated subsystems for industrial requirements engineering.\", \"page\": 5, \"index\": 1, \"width\": 767, \"height\": 1115}]"
motivation: 工业4.0环境下处理海量且异构的非结构化需求文档效率低下，且缺乏针对真实工业流程的RAG性能评估。
method: 开发了一个集成混合语义-词法检索与多供应商LLM编排的RAG框架，并在真实的汽车制造需求文档和供应商资质数据上进行实证评估。
result: "该框架实现了98.2%的提取准确率，减少了83%的人工分析时间，并识别出20.4%的供应商需重新认证，潜在节省230万美元违约金。"
conclusion: RAG框架在工业级需求工程中表现卓越，能有效平衡提取精度、可追溯性与成本效益，是实现工业文档自动化处理的可靠方案。
---

## 摘要
工业 4.0 中的需求工程面临着异构、非结构化文档的严峻挑战，这些文档涵盖了技术规范、供应商名单和合规标准。尽管检索增强生成 (RAG) 在知识密集型任务中展现出潜力，但此前尚无研究使用全面的生产级性能指标，在真实的工业需求工程 (RE) 工作流上对 RAG 进行评估。本文针对工业需求工程自动化，对 RAG 进行了全面的实证评估，使用了真实的汽车制造文档，包括 2015 年至 2023 年间四个规范标准（MBN 9666-1、MBN 9666-2、BQF 9666-5、MBN 9666-9）下的 669 条需求，以及 49 份带有大量支持文档的供应商资质。通过与基于 BERT 和无基准（ungrounded）大语言模型 (LLM) 方法的受控对比，该框架实现了 98.2% 的提取准确率和完整的可追溯性，分别比基准模型高出 24.4% 和 19.6%。混合语义-词法检索实现了 0.847 的平均倒数排名 (MRR)。专家质量评估在五个维度上的平均得分为 4.32/5.0。评估表明，通过多供应商 LLM 编排，手动分析时间减少了 83%，成本节省了 47%。消融实验量化了各组件的贡献。纵向分析显示，需求量减少了 55%，同时对 IT 安全的关注度增加了 1800%，识别出 10 家需要重新认证的遗留供应商（占比 20.4%），这代表了约 230 万美元的潜在合同罚金规避。
