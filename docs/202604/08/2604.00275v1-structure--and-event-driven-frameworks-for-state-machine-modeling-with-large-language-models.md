...
tldr: 本研究探讨了利用大语言模型（LLM）从非结构化自然语言需求中自动生成UML状态机的能力。研究提出了结构驱动和事件驱动两种建模框架，并引入了一种混合方法，通过初始草案加框架优化的方式提升生成质量。实验评估了GPT-4o和Claude 3.5 Sonnet的表现，结果显示LLM在识别状态和转换方面表现出色，但在处理守卫条件和动作方面仍有局限，为自动化状态机设计提供了重要基准。
source: arxiv
...
motivation: 传统的UML状态机设计依赖人工且耗时，而现有自动化方法通常需要结构化的输入，限制了其在非结构化需求下的应用。
method: 提出了结构驱动和事件驱动两种状态机框架，并结合单步提示与多步细化的混合方法来优化LLM的生成过程。
result: 实验表明Claude 3.5 Sonnet在状态和转换识别上表现优异，而混合方法显著提升了非推理型模型GPT-4o的性能。
conclusion: 虽然LLM在自动化状态机建模中展现出巨大潜力，但在复杂逻辑组件的生成上仍需进一步改进，目前尚不能完全取代人工设计。
---

## 摘要
UML 状态机设计是软件工程中的一个关键过程。传统上，状态机由经验丰富的工程师根据自然语言需求手动构建，这是一个耗时且易出错的过程。尽管存在许多自动化方法，但它们通常需要结构化的自然语言需求。在本文中，我们研究了当前大语言模型（LLM）通过专门的状态机框架（SMF）从非结构化自然语言需求中全自动生成 UML 状态机的能力。我们使用单步和多步提示方法评估了两种最先进的 LLM：非推理型 LLM GPT-4o 和侧重推理的 LLM Claude 3.5 Sonnet，并引入了一种新颖的混合方法，该方法将单提示基线的输出作为初始草图状态机，随后通过 SMF 进行细化。在我们的研究中，基于人类方法开发了两种不同的 SMF：(i) 结构驱动型 SMF，其中状态机组件（状态、转换、守卫、动作等）按顺序步骤生成；(ii) 事件驱动型 SMF，通过识别出的事件迭代地指导状态机构建。实验表明，虽然 LLM 在单提示基线下表现出生成状态机模型的潜力（例如，使用 Claude 3.5 Sonnet 时，状态的 F1 分数为 0.90，转换的 F1 分数为 0.75），但其性能尚不足以支持全自动化解决方案（例如，GPT-4o 在守卫方面的 F1 分数为 0.23，在动作方面为 0.00）。我们提出的混合方法将非推理型 LLM (GPT-4o) 的性能提升到了与推理型 LLM (Claude 3.5 Sonnet) 相似的水平，但未能进一步提升推理型 LLM 的表现。我们的评估突显了当前 LLM 在自动化状态机设计方面的潜力和局限性，为该领域的未来研究提供了基准。
