...
title_zh: 表征大语言模型生成的胜任力问题：一项使用开源和闭源模型的跨领域实证研究
authors: "Reham Alharbi, Valentina Tamma, Terry R. Payne, Jacopo de Berardinis"
date: 2026-04-17
pdf: "https://arxiv.org/pdf/2604.16258v1"
tags: ["query:re-us"]
score: 7.0
evidence: 使用自然语言问题的需求获取
tldr: 本研究探讨了利用大语言模型（LLM）自动生成本体工程中的能力问题（CQs）。通过对开源（如Llama 3.1）和闭源（如GPT-4.1）模型在多领域下的表现进行系统化实证研究，提出了一套涵盖可读性、相关性和结构复杂度的定量评估指标。研究揭示了不同模型在处理特定用例时表现出的独特生成特征，为自动化本体需求获取提供了重要参考。
source: arxiv
...
motivation: 旨在通过系统化分析，理解不同规模和类型的LLM在自动生成本体能力问题（CQs）时的内在属性和表现差异。
method: 提出了一套定量衡量标准，并在多个领域用例下对比了多种开源与闭源模型生成的CQs在可读性、相关性和结构复杂度上的表现。
result: 实验表明，LLM生成的CQs表现出受具体用例驱动的独特生成特征，且不同模型在各项评估维度上存在显著差异。
conclusion: 该研究证明了利用LLM自动化生成CQs的可行性，并强调了根据具体应用场景选择合适模型的重要性。
---

## 摘要
胜任力问题（Competency Questions, CQs）是本体工程中需求获取的基石。CQs 将需求表示为本体应满足的一组自然语言问题；传统上，它们由本体工程师与领域专家共同建模，作为以人为中心的、手动获取过程的一部分。生成式人工智能的使用实现了大规模的 CQ 自动创建，从而使生成过程民主化，扩大了利益相关者的参与，并最终拓宽了本体工程的准入门槛。然而，鉴于大语言模型（LLMs）庞大且异构的格局，它们在参数规模、任务和领域专业化以及可访问性等维度上各不相同，因此通过系统的跨领域分析来表征和理解它们生成的 CQs 的内在可观察属性（如可读性、结构复杂度）至关重要。本文引入了一套定量指标，用于在多个维度上对 CQs 进行系统比较。利用从定义明确的用例和场景中生成的 CQs，我们识别了它们的显著属性，包括可读性、与输入文本的相关性以及生成问题的结构复杂度。我们在一系列用例和需求上使用多种 LLMs 进行了实验，包括开源模型（KimiK2-1T, LLama3.1-8B, LLama3.2-3B）和闭源模型（Gemini 2.5 Pro, GPT 4.1）。我们的分析表明，LLM 的性能反映了由用例塑造的独特生成特征。
