...
tags: ["query:re-us"]
score: 7.0
evidence: 协作规范和上下文工程方法论
tldr: 本研究针对AI编程中过度追求速度而忽视准备的“氛围编程”现状，提出了名为MEP的上下文工程方法论。该方法借鉴烹饪中的“就绪”概念，通过上下文衔接、协作规范和任务分解三个阶段，将人类的隐性知识转化为结构化上下文。实验证明，充分的准备能显著提升AI智能体的开发效率，减少调试成本，并提出了“上下文流畅度”这一新型开发者技能。
source: arxiv
...
motivation: 旨在解决AI编程智能体因缺乏充分上下文而导致代码质量低、调试和重构成本高的问题。
method: 提出了包含上下文衔接、协作规范和任务分解三个阶段的MEP（Mise en Place）准备方法论。
result: 在黑客松实践中，仅需两小时准备即可支持AI智能体并行实现全栈教育平台的快速开发。
conclusion: 强调了“上下文流畅度”是AI时代开发者的核心技能，并为AI辅助开发的准备阶段研究提供了新框架。
---

## 摘要
AI 编程智能体的快速普及产生了一种主导性的工作流模式——通常被称为“氛围编程”（vibe coding）——它优先考虑实现速度而非刻意准备。我们认为，这种方法产生了一个系统性的对齐问题：缺乏足够上下文的智能体生成的代码需要大量的调试和重构，消耗了大量的开发时间。借鉴烹饪中的 Mise en Place（一切就绪；简称 MEP）概念，我们为智能体编程提出了一种三阶段准备方法论：(1) 上下文锚定（contextual grounding），将领域专业知识和隐性知识外显化为结构化文档；(2) 协作规范（collaborative specification），通过人机对话生成详细的设计产物；以及 (3) 任务分解（task decomposition），将规范转换为结构化的、具有依赖感知能力的任务记录。我们报告了 MEP 在一场竞争激烈的黑客松中的应用情况，其中大约两小时的准备工作使得并发 AI 智能体能够快速并行实现一个全栈教育平台。我们引入了“上下文流利度”（context fluency）这一概念，将其作为一种新兴的开发者技能——即创建智能体可操作的丰富、结构化上下文的能力——并将其与逆向设计（backward design）和隐性知识外显化中的既有框架联系起来。最后，我们提出了一个研究议程，旨在实证验证 AI 辅助软件开发中准备阶段方法论的有效性。
