...
tags: ["query:re-us"]
score: 6.0
evidence: 从议题讨论中自动提取知识
tldr: 针对大规模开源软件项目中GitHub Issue讨论冗长且碎片化的问题，本文提出了SWE-MIMIC-Bench，一个通过多LLM流水线自动生成的推理轨迹数据集。该流水线利用多个闭源LLM执行评论分析、外部资源总结及标签分类等细粒度任务，将非结构化讨论转化为包含根因、方案和进度的结构化叙事。该研究不仅能辅助开发者快速理解问题诊断过程，还为训练专家级LLM智能体提供了高质量的推理数据。
source: arxiv
...
motivation: 解决开源软件维护中，开发者难以从冗长、非结构化的GitHub Issue讨论中高效提取关键解决思路和知识的问题。
method: 采用多LLM协作流水线，通过对评论进行细粒度分析、外部链接总结及标签化合成，将原始讨论转化为结构化的推理轨迹。
result: "在来自SWE-Bench系列的800个真实问题上，该系统以91.7%的成功率提取了734条高保真推理轨迹。"
conclusion: 该方法能有效提取社区知识，生成的结构化轨迹对于提升开发者效率和训练高性能软件工程LLM智能体具有重要价值。
---
