...
tags: ["query:re-us"]
score: 6.0
evidence: 从非结构化问题讨论中自动提取知识
tldr: 针对大规模开源项目中 GitHub Issue 讨论冗长且非结构化导致开发者认知负担重的问题，本文提出了 SWE-MIMIC-Bench。该研究利用多 LLM 流水线，通过对评论进行细粒度分析、外部资源关联及标签化分类（如根因分析、解决方案等），将碎片化的讨论转化为结构化的推理轨迹。该方法不仅能辅助开发者快速理解问题诊断过程，还为训练专家级软件工程 LLM 智能体提供了高质量的经验轨迹数据集。
source: arxiv
...
motivation: 旨在解决开源软件开发中，从冗长且碎片化的 GitHub Issue 讨论中提取有效解决问题的知识和推理路径的难题。
method: 提出一种多 LLM 协作流水线，通过评论分析、标签分类和轨迹合成，将原始讨论转化为包含根因和方案的结构化叙述。
result: "在 800 个真实 GitHub Issue 上进行了评估，实现了 91.7% 的提取成功率，生成了 734 条高保真推理轨迹。"
conclusion: 该研究为社区知识提取提供了高效工具，并为提升 LLM 智能体在复杂软件工程任务中的推理能力奠定了数据基础。
---
