...
## 摘要
将非结构化自然语言需求自动翻译为形式化规范，在需求工程和形式化方法领域仍是一项挑战，特别是对于验证依赖于数学精确规范的安全关键型和任务关键型系统。本文评估了当代现成的大语言模型（LLMs）是否能通过直接从非结构化需求生成线性时序逻辑（LTL）公式来帮助弥合这一差距。该研究采用少样本提示策略，在包含 15 个结构各异的需求的异构基准测试集上考察了六种现代 LLM。每个“需求-模型”对都收集了五次独立生成结果，总共产生了 450 个候选 LTL 公式。性能评估通过人工语义评估、pass@k（k ∈ {1, 3, 5}）以及捕捉随机试验中语法可复现性的自一致性指标进行。结果表明，当前的通用 LLM 在无需特定任务微调的情况下，就能在非结构化自然语言到 LTL 的转换任务上取得具有实际意义的性能。该研究还通过将生成的公式与模型产生的自然语言解释相结合，并讨论了基于时间线的 LTL 可视化的补充用途，考虑了非专家的可理解性。研究结果表明，现代 LLM 正在成为半自动化形式化工作流中可行的前端助手。
