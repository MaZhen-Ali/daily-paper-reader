...
tldr: 尽管敏捷开发优于传统方法，但其成功率仍有待提高。本研究采用实证主义方法，通过PLS-SEM结构方程模型分析了敏捷从业者的感知数据，旨在识别影响敏捷软件开发项目成功的关键成功因素（CSF）。研究发现少数关键因素对项目成功有显著影响，并据此构建了一个用于评估和衡量项目成功的新型模型，为提升敏捷项目管理效能提供了理论与实践指导。
source: arxiv
...
motivation: 针对敏捷软件开发项目成功率依然偏低且缺乏系统性关键成功因素评估的问题展开研究。
method: 采用实证主义研究方法，利用SmartPLS软件进行偏最小二乘结构方程模型（PLS-SEM）分析。
result: 识别出了对敏捷项目成功具有显著贡献的少数几个关键核心因素。
conclusion: 开发了一个能够有效评估和衡量敏捷项目成功的新型预测模型。
---

## 摘要
尽管敏捷项目比传统的软件开发项目管理方法更成功，但其整体成功率仍然相对较低，仍有很大比例的项目被认为面临挑战或失败。这种较低的项目成功率归因于项目未能根据敏捷项目的关键成功因素（CSF）和当代项目成功标准进行严格评估。为了识别敏捷从业者所感知的有助于敏捷软件开发项目成功的关键成功因素，本研究采用实证主义方法对敏捷软件开发项目的关键成功因素进行了调查。研究使用 SmartPLS 进行偏最小二乘结构方程模型（PLS-SEM）分析，以分析数据并验证假设，从而识别构念与项目成功标准之间的显著关系。本研究发现，少数几个关键因素对敏捷项目的成功有显著贡献。该研究开发了一个可用于评估和衡量项目成功的新型模型。
