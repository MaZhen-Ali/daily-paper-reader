...
## 摘要
尽管敏捷项目比传统的软件开发项目管理方法更成功，但其整体成功率仍然相对较低，仍有很高比例的项目被认为面临挑战或失败。这种较低的项目成功率归因于项目未能根据敏捷项目的关键成功因素（CSF）和当代项目成功标准进行严格评估。为了识别敏捷从业者所认知的有助于敏捷软件开发项目成功的关键成功因素，本研究采用实证主义方法对敏捷软件开发项目的关键成功因素进行了调查。研究使用 SmartPLS 进行偏最小二乘结构方程模型（PLS-SEM）分析，以分析数据并验证假设，从而识别构念与项目成功标准之间的显著关系。研究发现，少数几个关键因素对敏捷项目的成功有显著贡献。本研究开发了一个可用于评估和衡量项目成功的新型模型。
